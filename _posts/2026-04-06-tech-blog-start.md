...
솔직히 말하면, 처음부터 쓸만한 아키텍처를 설계하고 싶었다기보다는 한 파일이 너무 커지는 게 싫었다. 예제 수준에서는 한 파일 안에 다 넣어도 되지만, 조금만 엔진 흉내를 내기 시작하면 금방 감당이 안 되기도 하니까.
...
물론 이 구조가 최종 형태라는 뜻은 아니다. 오히려 지금은 앞으로 바뀔 것을 전제로 한 첫 번째 버전에 가깝다. 그래도 적어도 지금 단계에서 무언가를 추가하거나 고칠 때, 어디를 건드려야 하는지는 조금씩 선명해지고 있다.
...
악명이 높아서인지 벌칸 2026 로드맵에 Descriptor Heaps 관련 내용도 있으니 기대해봐도 좋을 것 같다.
